...
class Network:
    ...
    def evaluate(self, raster_plot_interval, firing_rates_interval,tw):
        ...
        if nest.Rank() == 0:
            ...
            helpers.plot_raster(
                ...
                self.net_dict['N_scaling'],self.net_dict['populations'])
            
            helpers.plot_average_Vm(self.data_path, 'voltmeter', 500,
                600,
                self.net_dict['N_scaling'],self.net_dict['populations'])
...
#                 firing_rates_interval[0], firing_rates_interval[1])
            ...
            self.response_time,self.RT_start,self.response_rate,self.rest_rate = helpers.firing_rates_segment_plot(self.data_path, "spike_recorder", raster_plot_interval[0], raster_plot_interval[1], tw, self.net_dict['th_response'],[self.stim_dict['th_start']],self.net_dict['populations'])
    ...
    def __create_neuronal_populations(self):
        ...
        for i in np.arange(self.num_pops):
            ...
            population.set(
                ...
                E_L=self.net_dict['neuron_params']['E_L'][i],
                V_th=self.net_dict['neuron_params']['V_th'][i],
                V_reset=self.net_dict['neuron_params']['V_reset'][i],
                t_ref=self.net_dict['neuron_params']['t_ref'][i],
                I_e=self.DC_amp[i])
    ...
    def __create_recording_devices(self):
        ...
        if 'voltmeter' in self.sim_dict['rec_dev']:
            ...
            vm_dict = {'interval': self.sim_dict['rec_V_int'],
                       'record_to': 'ascii',
                       'record_from': ['V_m', 'I_syn_in','I_syn_ex'],
                       'label': os.path.join(self.data_path, 'voltmeter')}
            self.voltmeters = nest.Create('multimeter',
                                          n=self.num_pops,
                                          params=vm_dict)
    ...
    def __create_poisson_bg_input(self):
        ...
        print('br_rate is equal to '+ str(self.net_dict['bg_rate']))
    ...
    def __connect_neuronal_populations(self):
        ...
        pprint.pprint(self.num_synapses)
