...
def plot_raster(path, name, begin, end, N_scaling,ylabels):
    ...
    """
    fs = 7  # fontsize
    color_list = np.array(['#595289','#595289','#595289'])
    for i in range(len(ylabels)-3):
        color_list=np.append(color_list,'#af143c')
    ...
    label_pos = [(mod_node_ids[i, 0] + mod_node_ids[i, 1]) / 2.
                 for i in np.arange(0, len(ylabels))] #edited by qujingling
...
def firing_rates_segment_plot(path, name, begin, end, segment, threshold, sim_time, ylabels):
    """ Computes mean and standard deviation of firing rates per population.
    ...
    begin
        ...
        Time point (in ms) to stop calculating the firing rates (included).
    segment
        Time segment (in ms) to calculating the firing rates mean (included).
    threshold
        firing rates threshold: [460., 420., 490., 370.], 1xN matrix
    sim_time
        simulate time (ms): [700]
    ylabels
        matplotlib ylabels: ['L23', 'L4', 'L5', 'L6']
    Returns
    -------
    response_time, all_rates
    ...
    for i, n in enumerate(sd_names):
        ...
        senders = data[i]['time_ms']
        bins = np.arange(begin, end + 1, segment)
        spike_count_per_neuron, _ = np.histogram(senders, bins=bins)
        rate_per_neuron = spike_count_per_neuron * 1000. / (segment*(node_ids[i,1]-node_ids[i,0]+1))
        weights = np.hanning(5)
        rate_per_neuron = np.convolve(weights/weights.sum(), rate_per_neuron,'same')
        all_rates.append(rate_per_neuron)

        # calculate response time
        resp_index = np.where(rate_per_neuron>=threshold[i])[0]                           # rate_per_neuron > threshold's index
        rest_index = np.where(np.arange(begin, end, segment)< sim_time[0])[0]
        rest_rates.append(np.mean(rate_per_neuron[rest_index[-1-round(10/segment)]:rest_index[-1]]))
        
        if len(resp_index) == 0:
            print("\033[1;32;43m Warning! \033[0m! The {} is no response: {}".format(ylabels[i], resp_index))
            response_time.append(-1)
            RT_start.append(-1)
            response_rates.append(-1)
            continue

        sim_index = np.where(np.arange(begin, end, segment)>=sim_time[0])[0]                     # simulate time index
        resp_index = [item for item in resp_index.tolist() if item in sim_index.tolist()] # remove not in simulate time index
        nresp_time = (resp_index[-1]*segment+begin)-(resp_index[0]*segment+begin)
        nRT_start = resp_index[0]*segment     
        response_time.append(nresp_time)
        RT_start.append(nRT_start)
        # calculate response firing rate
        response_rates.append(np.mean(rate_per_neuron[resp_index[0]:resp_index[-1]+1]))
    plt.figure()
    print('response _time')
    response_time=np.round(np.array(response_time),8)
    print(response_time)
    print('RT_start')
    RT_start = np.round(np.array(RT_start),8)
    print(RT_start)
    print('response_rate')
    response_rate = np.round(np.array(response_rates),8)
    print(response_rate)
    print('rest_rate')
    rest_rate = np.round(np.array(rest_rates),8)
    print(rest_rate)
    print('all_rates')
    print(all_rates)
    for i, rate in enumerate(all_rates):
        plt.plot(np.arange(begin, end+1, segment)[0:-1:1], all_rates[i])
    plt.legend(ylabels, loc='upper right')  
    plt.savefig(os.path.join(path,'different_time_{}ms_seg_firing_rate_plot.png').format(segment), dpi=300)
    
    return response_time,RT_start,response_rate,rest_rate
    ...
    for i, name in enumerate(sd_names):
        ...
        for j, f in enumerate(sd_files):
            if name in f:
                ...
                for itrL,line in enumerate(open(os.path.join(path, f))):   
                    if itrL<3:
                        continue
                ...
                ld = np.genfromtxt(os.path.join(path, f), skip_header=itrL,dtype=dtype,invalid_raise=False)    
...
def __load_vm_times(path, name, begin, end):
    """ Loads Vm times of each voltmeter.

    Parameters
    ----------
    path
        Path where the files with the vm are stored.
    name
        Name of the voltmeter.
    begin
        Time point (in ms) to start loading vm (included).
    end
        Time point (in ms) to stop loading vm (included).

    Returns
    -------
    data
        Dictionary containing vm in the interval from ``begin``
        to ``end``.

    """
    sd_files, sd_names, node_ids = __gather_metadata(path, name)
    dtype = {'names': ('sender', 'time_ms','V_m', 'I_syn_in','I_syn_ex'),  # as in header
    ...
    for i, name in enumerate(sd_names):
        ...
        low = np.searchsorted(data_i_raw['time_ms'], v=begin, side='left')
        high = np.searchsorted(data_i_raw['time_ms'], v=end, side='right')
        data = np.append(data,data_i_raw[low:high])
    return sd_names, node_ids, data


def plot_average_Vm(path, name,begin, end, N_scaling,ylabels):
    fs = 18  # fontsize
    # color_list = np.tile(['#595289', '#af143c'], 4)
    
    sd_names, node_ids, data = __load_vm_times(path, name, begin, end)
    last_node_id = node_ids[-1, -1]
    mod_node_ids = np.abs(node_ids - last_node_id) + 1
    colorlist = ['#DC143C','#EE82EE','#4B0082','#6495ED','#87CEEB','#00FA9A','#FFFF00','#FFA500','#A9A9A9','#F08080','#D2B48C','#00FFFF']
    stp = 1
    if N_scaling > 0.1:
        stp = int(10. * N_scaling)
        print('  Only spikes of neurons in steps of {} are shown.'.format(stp))
    data = np.sort(data, order=['sender'])
    ...
    plt.figure(figsize=(8, 6))
    ax3 = plt.gca()
    for i in np.arange(0, len(ylabels)):
        high = np.searchsorted(data['sender'], v=mod_node_ids[i, 0], side='right')
        low = np.searchsorted(data['sender'], v=mod_node_ids[i, 1], side='left')
        print(np.size(np.where(data['time_ms']==650)))
        data_tmp = data[low:high]
        times = []
        Vm = []
        I_syn_in = []
        I_syn_ex = []
        for j,t in enumerate(np.unique(data['time_ms'])):
            times.append(t)
            itr = np.where(data_tmp['time_ms']==t)
            Vm.append(np.mean(data_tmp[itr]['V_m']))
            I_syn_in.append(np.mean(data_tmp[itr]['I_syn_in']))
            I_syn_ex.append(np.mean(data_tmp[itr]['I_syn_ex']))
        plt.sca(ax1)
        plt.plot(times[::stp], Vm[::stp],label=ylabels[i],color=colorlist[i])
        plt.sca(ax2)
        plt.plot(times[::stp], I_syn_in[::stp],label=ylabels[i],color=colorlist[i])
        plt.sca(ax3)
        plt.plot(times[::stp], I_syn_ex[::stp],label=ylabels[i],color=colorlist[i])
    plt.sca(ax1)
    plt.xlabel('time [ms]', fontsize=fs)
    plt.ylabel('V_m[mV]', fontsize=fs)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.legend()
    plt.savefig(os.path.join(path,'average_voltage_plot.png'), dpi=300)
    plt.sca(ax2)
    plt.xlabel('time [ms]', fontsize=fs)
    plt.ylabel('I_syn_in[mA]', fontsize=fs)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.legend()
    plt.savefig(os.path.join(path,'average_I_syn_in_plot.png'), dpi=300)
    plt.sca(ax3)
    plt.xlabel('time [ms]', fontsize=fs)
    plt.ylabel('I_syn_ex[mA]', fontsize=fs)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.legend()
    plt.savefig(os.path.join(path,'average_I_syn_ex_plot.png'), dpi=300)